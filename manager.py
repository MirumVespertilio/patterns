...
class OrderManager:
    ...
    1. Единственный экземпляр класса на всё приложение
    2. Глобальную точку доступа к этому экземпляру
    
    Это важно для кофейни, так как:
    - Все заказы должны храниться в одном месте
    - Нужна единая нумерация заказов
    - Исключаются конфликты при работе с данными
    """
